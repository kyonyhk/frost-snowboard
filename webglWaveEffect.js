...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const circleTexture = createCircleTexture(64, '#6BE688');
  ...
  window.addEventListener('deviceorientation', (event) => {
    if (event.gamma !== null && event.beta !== null) {
      const maxTilt = 30; // Maximum tilt value for gamma and beta
      mouse.x = event.gamma / maxTilt; // gamma is the left-to-right tilt in degrees
      mouse.y = -(event.beta / maxTilt); // beta is the front-to-back tilt in degrees
    }
  });
